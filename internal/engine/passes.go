...
func (e *Engine) claimAndDispatch(ctx context.Context, c store.Candidate) {
	// The executor is chosen BEFORE the claim, because dispatched_to is written in the claim
	...
	target, err := e.pickExecutor(ctx, def)
	if err != nil {
	...
	if target != nil {
		p.ExecutorID = target.ExecutorID
	}

	res, err := e.store.Claim(ctx, p, e.runnableFor(target))
	...
	e.dispatch(ctx, h, res.Definition, *target, p)
}
...
func (e *Engine) runnableFor(target *store.Executor) store.Runnable {
...
// pickExecutor chooses where to send an execution.
//
// Least-loaded among the live candidates, with the advisory `running` count as the signal.
// It is advisory on purpose: the executor's own refusal is authoritative, and treating a
// stale count as a hard limit would leave capacity unused whenever a heartbeat is a second
// old. Ties break on the freshest heartbeat, so a fleet that is genuinely idle spreads out.
func (e *Engine) pickExecutor(ctx context.Context, def gojob.Definition) (*store.Executor, error) {
	...
	for i := 1; i < len(live); i++ {
		if headroom(live[i]) > headroom(live[best]) {
			best = i
		}
	}
	pick := live[best]
...
func (e *Engine) releaseUndispatchable(ctx context.Context, p store.ClaimParams, epoch int64, why string) {
	...
	e.untrack(p.ExecutionID)
	e.log.Warn("execution returned to ready", "job", p.JobName,
		"execution", p.ExecutionKey, "reason", why)
}

// dispatch sends the Run call and applies its outcome.
func (e *Engine) dispatch(ctx context.Context, h store.Holder, def gojob.Definition, target store.Executor, p store.ClaimParams) {
	params, err := decodeParams(def.Params)
	if err != nil {
		e.log.Error("a job's stored parameters are not a JSON object",
			"job", def.JobName, "error", err)
	...
	for attempt := 0; ; attempt++ {
		...
		case dispatch.Accepted:
			...
			// happened and then wait for a result that belongs to someone else.
			if res.HeldToken != "" && res.HeldToken != h.RunToken {
				...
				e.releaseUndispatchable(ctx, p, h.FenceEpoch, "executor holds an older attempt")
				return
			}
		...
		case dispatch.Refused:
			e.log.Info("executor refused a dispatch", "job", def.JobName,
				"execution", h.ExecutionKey, "executor", target.ExecutorID, "code", res.Code)
			e.releaseUndispatchable(ctx, p, h.FenceEpoch, "executor refused: "+res.Code.String())
			return
		...
		default:
			...
			if attempt+1 >= e.cfg.DispatchResendLimit || time.Now().After(deadline) {
				e.log.Warn("dispatch outcome unknown after the re-send bound; leaving it to recovery",
					...
					"attempts", attempt+1, "code", res.Code)
				// Stop renewing: recovery reconciles with the executor and resolves it like any
				// other unknown attempt. Continuing to renew is what would make it permanent.
				e.untrack(h.ExecutionID)
				return
			}
			select {
...
func (e *Engine) fenceTimedOut(ctx context.Context, v store.Stale) {
	...
	if v.DispatchedTo != "" {
		if addr, ok := e.addressOf(ctx, v.DispatchedTo); ok {
			if err := e.disp.Cancel(ctx, addr, e.cfg.Tenant, v.ExecutionKey, v.RunToken,
				"runtime cap elapsed"); err != nil {
				e.log.Debug("cancel of a timed-out execution was not acknowledged",
					"execution", v.ExecutionKey, "error", err)
			}
		}
	}
